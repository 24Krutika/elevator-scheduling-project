...
public class PaceLift {

   public static void main(String[] args) {
       System.out.println("Welcome to Pace Lift");
       
       Thread requestListenerThread = new Thread(new RequestListener(),  //used to read floor
               "RequestListenerThread");
       
       Thread requestProcessorThread = new Thread(new RequestProcessor(),  //process requested floor
...
class Elevator {
   ...
   static Elevator getInstance() {                  //elevator getInstance
       if (elevator == null) {
   ...
   public synchronized void addFloor(int f) {
       ...
       if(requestProcessorThread.getState() == Thread.State.WAITING){
           ...
           notify();            //notify the processor
       }else{
            
           requestProcessorThread.interrupt();  //check for interruptions
       }
   ...
   public synchronized int nextFloor() {
       ...
       if (floor == null) {
           ...
           requestSet.remove(floor);    //removes the requested floor when it is processed
       }
   ...
   public void run() {
       ...
       while (true) {
           ...
           try {
               ...
               BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(System.in));    //reads input from the user
